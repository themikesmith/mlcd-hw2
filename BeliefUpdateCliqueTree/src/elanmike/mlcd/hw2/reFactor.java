...
public class reFactor {
	...
	protected reFactor(ArrayList<? extends Variable> vars){
	...
	protected reFactor(ArrayList<? extends Variable> vars, double d) {
		_variables= new ArrayList<LocalVariable>(vars.size());
		for(int index=0; index < vars.size(); index ++) 
			_variables.add(new LocalVariable(vars.get(index),-1));
	...
	protected reFactor(reFactor factToCopy){
...
//				output += String.format("%01d\t", values.get(var_idx));
	...
	public reFactor product(reFactor f) throws ArrayIndexOutOfBoundsException {
		ArrayList<Variable> unionScope = this.union(f._variables);
		reFactor psi = new reFactor(unionScope);
	...
	public reFactor divide(reFactor f) throws FactorScopeException, FactorIndexException {
		...
		reFactor result = new reFactor(this._variables);
	...
	//example p297
	public reFactor marginalize(ArrayList<Variable> elimVar) throws FactorIndexException{
		ArrayList<Variable> finalVars = difference(elimVar);
		
		reFactor result = new reFactor(finalVars, 0);
...
//					Math.exp(result.data.get(result.index(f_indicies_of_values))));
	...
	public reFactor reduce(ArrayList<Variable> heldVars, ArrayList<Integer> heldValues){
		reFactor result = new reFactor(this.difference(heldVars));
	...
	public static reFactor indicatorFunctionForNames(ArrayList<String> varNames, ArrayList<String> valueNames){
	...
	public static reFactor indicatorFunction(ArrayList<HeldVariable> vars){
		reFactor result = new reFactor(vars,Math.log(0.0));
